...
img_ori = cv2.imread('./car_num01.jpg')
...
for d in contours_dict:
    area = d['w']*d['h']
    ratio = d['w']/d['h']
    if area> MIN_AREA and d['w']>MIN_W and d['h']>MIN_H  and MIN_R<ratio and ratio<MAX_R:
        d['idx'] = cnt
        cnt+=1
        possible_contours.append(d)
temp_res = np.zeros((height,width,channel),dtype=np.uint8)
for d in possible_contours:
    cv2.rectangle(temp_res,pt1=(d['x'],d['y']),pt2=((d['x']+d['w']),(d['y']+d['h'])),color = (255,255,255),thickness=2)


MAX_DIA_MUL =5
## Two Contours Between'length diff
MAX_ANGLE_DIFF=12.0
MAX_AREA_DIFF=0.5
MAX_WIDTh_DIFF=0.8
MAX_HEIGHT_DIFF = 0.2
...
def find_chars(contour_list):
    ...
    for d1 in contour_list:
        ...
        for d2 in contour_list:
            ...
            if distance < diagonal_length1* MAX_DIA_MUL and angle_diff<MAX_ANGLE_DIFF \
