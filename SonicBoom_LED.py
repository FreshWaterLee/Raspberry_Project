...
distances = [Jug_dis+1,0,0] ## 순서대로 정면, 왼쪽, 오른쪽 전역변수로 선언한 이유는 계속해서 변하기 때문
...
def Led_Jud(): ## 이동 가능 여부 확인용 
    ...
    if(Jug_dis<distances[0]):
        ...
        if(Jug_dis<distances[lr_idx] and distances[0]<distances[lr_idx]): ## 직진만 할것인지 좌,우 회전을 해야하는지
            # 좌우에 공간 여유가 적다면 좌우 코너링은 안함(직진만),
            #또한 정면 공간보다 여유가 적다면 굳이 코너링을 할 필요는 없다
            print("좌우 회전!!")
